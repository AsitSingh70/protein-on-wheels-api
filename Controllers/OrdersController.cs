...
public class OrdersController : ControllerBase
{
    ...
    public IActionResult PlaceOrder([FromBody] CreateOrderDTO dto)
    {
        ...
        // 🔥 GIFT LOGIC START  //added on 02-05-26
        var gifts = new List<string>();

        if (order.TotalAmount > 5500)
        {
            gifts = new List<string> { "Neckband", "Watch", "Massager", "Juice Grinder","Creatine 122gm","Lamp" };
        }
        else if (order.TotalAmount > 2500)
        {
            gifts = new List<string> { "Watch", "Neck Band"};
        }

        if (gifts.Count > 0)
        {
            var random = new Random();
            order.GiftAssigned = gifts[random.Next(gifts.Count)];
        }
        else
        {
            order.GiftAssigned = "No Gift";   // ✅ FIX
        }
        // 🔥 GIFT LOGIC END
    ...
    public IActionResult GetOrders()
    {
        ...
        var result = orders.Select(o => new
        {
            ...
            o.PhoneNumber,
            o.Address,
            o.GiftAssigned, //added on 02-05-26
